...
@onready
var _stand: Texture2D = preload("res://assets/player/textures/stand.png")
...
func _process(delta: float) -> void:
	...
	var k := _parent_math.move_and_collide(
		Vector3.DOWN * 10 * delta, true, true, true
	)
	if k != null:
		if movement:
			hframes = 6
...
# Change the 2D basis of the sprite to try and make it "fit"
# multiple view modes.
...
# This method returns a bool but if true it also outputs to the
# direction variable.
func _check_movement() -> bool:
	...
	# For efficiency, only check the X axis since this X axis value
	# isn't used anywhere else.
	if (
		not _parent_math.isometric_controls
		and is_equal_approx(
			_parent_node25d.unit_scale * 0.86602540378,
			_parent_node25d.get_basis()[0].x
		)
	):
		if Input.is_action_pressed(&"move_right"):
			z += 1
