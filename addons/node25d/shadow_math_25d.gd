...
func _ready() -> void:
	...
	push_error(
		"Shadow is not in the correct place, ",
		"expected a previous sibling node with a 3D first child."
	)
...
func _physics_process(_delta: float) -> void:
	if _target_math == null:
		...
		# Shadow is not in a valid place or you're viewing the Shadow25D scene.
		return
