...
func node25d_ready():
	...
	_basis_y = SCALE * Vector2(0, -0.70710678118)
	_basis_z = SCALE * Vector2(0, 0.70710678118)
...
func set_view_mode(view_mode_index):
	match view_mode_index:
		0:  # 45 Degrees
			...
			_basis_z = SCALE * Vector2(0, 0.70710678118)
		1:  # Isometric
			_basis_x = SCALE * Vector2(0.86602540378, 0.5)
			_basis_y = SCALE * Vector2(0, -1)
			_basis_z = SCALE * Vector2(-0.86602540378, 0.5)
		2:  # Top Down
			_basis_x = SCALE * Vector2(1, 0)
		...
		4:  # Oblique Y
			_basis_x = SCALE * Vector2(1, 0)
			_basis_y = SCALE * Vector2(-0.70710678118, -0.70710678118)
			_basis_z = SCALE * Vector2(0, 1)
		5:  # Oblique Z
			_basis_x = SCALE * Vector2(1, 0)
			_basis_y = SCALE * Vector2(0, -1)
			_basis_z = SCALE * Vector2(-0.70710678118, 0.70710678118)
